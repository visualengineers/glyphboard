import { Component, Input, OnChanges, ElementRef, Output, EventEmitter,  ViewChild, OnInit, ViewEncapsulation} from '@angular/core';
...
export class DashboardFeatureConfigComponent implements OnInit, OnChanges {
  ...
  private static filtering(component: DashboardFeatureConfigComponent): void {
    ...
    for (let i = 0; i <= 1.001; i = i + steps) {
      if (relativeMinValue > i) {
        minValue = i;
      }
      if (relativeMaxValue <= i) {
        maxValue = i;
        break;
      }
    }
