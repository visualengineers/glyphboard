...
export class DotGlyph extends Glyph {
    ...
    public draw(position: any, features: any, progress?: number, isPassive?: boolean, isHighlighted?: boolean, animationProgress?: number) {
        ...
        progress = (progress || 1);
        let rad = this.dotConfig().drawAggregate ? position.weight / this.preferredRadius : this.preferredRadius;
        if (rad >= 8) { rad = 8; }
        ...
        if (isHighlighted) {
          ...
          this.context.fillStyle = 'rgba(' + inverseColorRGB[0]
            + ', '
            + inverseColorRGB[1]
            + ', '
            + inverseColorRGB[2]
        ...
        if (position.weight === undefined || position.weight === 1 || !this.dotConfig().drawAggregate) {
            this.context.fill();
        } else {
            this.context.stroke();
        }
